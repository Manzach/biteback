...
// ============================================================================
// Form screen for donors to post new food donation advertisements (UC-06)
// - Collects donation title, description, location, date, quantity
// - ✅ ADDS PHOTO UPLOAD via image_picker + Supabase Storage
// - Validates input before submission
// - Integrates with DonorProvider to save to Supabase
...
class _CreateDonationScreenState extends State<CreateDonationScreen> {
            ...
            Center(
              child: Column(
                children: [
                  ...
                  const Text(
                    '"Share extra food with the people in need"',
                    style: TextStyle(
                      fontSize: 14,
                ...
                children: [
                  ...
                  if (_selectedDate == null)
                    Padding(
                      ...
                      child: Text('Required', style: TextStyle(color: Colors.red[400], fontSize: 11)),
                    ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: (donor.isLoading || _isPickingPhoto) ? null : _submitDonation,
  ...
  // ==================================================================
  // ✅ HELPER: Pick Image from Gallery/Camera
  // ==================================================================
  ...
  // ==================================================================
  // ✅ HELPER: Submit Donation Form WITH PHOTO UPLOAD
  // ==================================================================
  Future<void> _submitDonation() async {
    ...
    if (_selectedDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select availability date')),
      );
      return;
    }
    ...
    if (userId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please login to donate')),
      );
      return;
    }

    // 4. Parse quantity safely
    ...
    // 6. Submit to DonorProvider WITH photo file
    final donorProvider = context.read<DonorProvider>();
    
    final success = await donorProvider.createDonation(
      donorId: userId,
      ...
      quantity: quantity,
      photoFile: _selectedPhoto, // ✅ PASS SELECTED PHOTO FILE
    );
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(donorProvider.errorMessage ?? 'Failed to post donation'),
          backgroundColor: Colors.red,
  ...
  // ==================================================================
  // CLEANUP: Dispose controllers to prevent memory leaks
  // ==================================================================
  @override
  void dispose() {
